		...
		success: function(data){
			...
			if(data.length > 0) { 
				$.each(data, function(index, item){
					html += "<tr style='width: 100%;' ondblclick='checkdDlclick(this)'  onclick='checkClick(this)'>"; 
...
function checkClick(e){
	//행 클릭 시 체크박스 체크
	var chkbox = $(e).find('td:first-child :checkbox');	//클릭이벤트가 일어난 행의 체크박스선택
	var disabled = $(e).find('td:first-child :checkbox').prop("disabled"); //비활성화 여부 체크
